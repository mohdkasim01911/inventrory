 <style>
   ...
   .user-initial-logo {
    width: 40px;
    height: 40px;
    background: #4B49AC; /* primary color */
    color: #fff;
    border-radius: 50%;
    display: flex;
    align-items: center;
 ...
 <nav class="navbar default-layout col-lg-12 col-12 p-0 fixed-top d-flex align-items-top flex-row">
        <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-start">
          ...
          <div>
          <a class="navbar-brand brand-logo user-logo" href="{{ route('dashboard') }}">
              <div class="user-initial-logo">
        ...
        <div class="navbar-menu-wrapper d-flex align-items-top">
          <ul class="navbar-nav">
            <li class="nav-item fw-semibold d-none d-lg-block ms-0">
              <h1 class="welcome-text">{{ now()->hour < 12 ? 'Good Morning' : (now()->hour < 17 ? 'Good Afternoon' : (now()->hour < 21 ? 'Good Evening' : 'Good Night')) }}, <span class="text-black fw-bold">{{Auth::user()->name}}</span></h1>
          ...
          <ul class="navbar-nav ms-auto">
            <li class="nav-item">
              <form method="GET" action="{{ route('dashboard') }}" id="yearForm">
                <select name="year" onchange="document.getElementById('yearForm').submit()" class="form-control">
